...
1. **kmp-pro** — repo-specific review against AGENTS.md.
...
2. **feature-dev:code-reviewer** (existing plugin agent — `subagent_type: feature-dev:code-reviewer`).
   Prompt: "Review the diff `<base>...HEAD` against project guidelines in AGENTS.md. Confidence ≥ 80. Report bugs, security, code quality.

   **Swift-interop checks specific to this repo (in addition to AGENTS.md §8):**
...
3. **modernization-scout** — pass through the general-purpose agent. The scout's job is to web-search for modernization opportunities specific to what changed in the diff. Prompt template (fill in `<diff-summary>` from a quick `git diff --stat`):
   ...
   > You are a Kotlin Multiplatform modernization scout. The diff under review touches: `<diff-summary>`. Read `gradle/libs.versions.toml` and AGENTS.md, then web-search for:
   >
   ...
   >
   > Report only items relevant to the diff or to existing code the diff touches. Each item: current version → proposed version → rationale → blast radius (just `:backgrounder`? whole build? CI? consumer iOS apps?). Don't edit `libs.versions.toml`.

Each agent returns a report. **Save each one** by writing to a temp findings directory:

```bash
mkdir -p .Codex/findings/round-<N>
```

The skill writes:
- `.Codex/findings/round-<N>/kmp-pro.md`
- `.Codex/findings/round-<N>/code-reviewer.md`
- `.Codex/findings/round-<N>/modernization.md`

(Use `Write` tool. Save the agents' returned text verbatim.)

### Phase 2 — Reconcile

Read all three reports. Produce `.Codex/findings/round-<N>/reconciled.md` with three sections:
...
Save its report to `.Codex/findings/round-<N>/docs-sync.md`.

### Phase 4 — Round summary

Append a short markdown section to `.Codex/findings/SUMMARY.md` (create if absent):
...
- If we're at the last round, conclude with: "Reports in `.Codex/findings/`. Review the latest `reconciled.md` and `docs-sync.md`."
...
- The skill writes to `.Codex/findings/` only. Add `.Codex/findings/` to `.gitignore` if it isn't already. (Check on first run; offer to add it.)
