...
│    Backgrounder — class with create / register / start / shutdown │
└──────────────────────────────────────────────────────────────────┘
        ▲                      ▲                      ▲
┌───────┴────────────┐ ┌───────┴────────┐ ┌───────────┴────────┐
...
- **One public surface, three actuals.** Consumers in `commonMain` write against `Scheduler` and `BackgroundWorker`. Each platform's `Backgrounder.Companion.create(...)` factory wires up its own `Scheduler` implementation behind that single interface, using plain constructor injection — no DI container required.
