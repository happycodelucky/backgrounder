...
- **One public surface, three actuals.** Consumers in `commonMain` write against `Backgrounder` (scheduling verbs + `runNow`), `BackgroundWorker`, and `BackgroundWorkerFactory`. Each platform's `Backgrounder.Companion.create(...)` factory wires up its own internal scheduler and runner implementations, using plain constructor injection — no DI container required.
...
- **Guarantees are honest.** `Backgrounder.guarantees()` returns a per-platform truth table; UX should branch on it rather than assume parity. See [Guarantees](guarantees.md). `runNow` makes a different (weaker) set of guarantees — see [Run now](../recipes/run-now.md).
